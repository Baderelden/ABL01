...
fingerprint.update(json.dumps([model, notes]).encode())
...
if 'report' in st.session_state:
    ...
    a, b, c = st.columns(3)
    a.metric('Damage size', size)
    b.metric('Confirmed damaged panels', count)
    c.metric('Estimated labour', hours)
    st.caption('Small: 1–2 panels • Medium: 3–5 • Large: 6+. This measures panel count, not damage severity.')
    ...
    payload = dict(report, panel_count=count, damage_size=size)
    payload.pop('case_id')
